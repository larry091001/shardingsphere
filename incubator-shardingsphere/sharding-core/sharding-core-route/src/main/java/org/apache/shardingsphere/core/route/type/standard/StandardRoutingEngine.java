...
import org.apache.shardingsphere.core.strategy.route.db.DatabaseShardingStrategy;
...
public final class StandardRoutingEngine implements RoutingEngine {
    ...
    private Collection<String> routeDataSources(final TableRule tableRule, final List<RouteValue> databaseShardingValues) {
        ...
        ShardingStrategy shardingStrategy = shardingRule.getDatabaseShardingStrategy(tableRule);
        Collection<String> result = null;
        if (shardingStrategy instanceof DatabaseShardingStrategy) {
        	result = ((DatabaseShardingStrategy) shardingStrategy).doSharding(availableTargetDatabases, databaseShardingValues, sqlStatement.getLogicSQL());
        }else {
        	result = shardingStrategy.doSharding(availableTargetDatabases, databaseShardingValues);
        }
